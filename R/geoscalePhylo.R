geoscalePhylo<-function(tree, ages, direction="rightwards",units=c("Period", "Epoch", "Age"), boxes="Age", tick.scale="myr", user.scale, cex.age=0.3, cex.ts=0.3, cex.tip=0.3, width=1, label.offset,ts.col=TRUE, vers="ICS2013", x.lim, quat.rm=FALSE,erotate,arotate,urotate,...){
  ...
  if(direction == "upwards"){
    ...
    par(fig=c(0,ts.width,0,1))
     par(mar=c(3,1,2,5))
      ...
      par(lend=1); plot.phylo(tree,plot=FALSE,label.offset=offset,no.margin=T,y.lim=x.lim,direction="upwards",cex=cex.tip,...)
          
        timescale.names <- timescale.rescaled
         timescale.names <- timescale.names[timescale.names[,"End"] > min(c(par()$usr[3],par()$usr[4])) & timescale.names[,"Start"] < max(c(par()$usr[3],par()$usr[4])),]
          timescale.names[timescale.names[,"End"] > max(c(par()$usr[3],par()$usr[4])),"End"] <- max(c(par()$usr[3],par()$usr[4]))
          timescale.names[timescale.names[,"Start"] < min(c(par()$usr[3],par()$usr[4])),"Start"] <- min(c(par()$usr[3],par()$usr[4]))
    ...
    for(t in 1:length(units)){
      ...
      if(ts.col == TRUE & units[t] != "User"){
        rect(tscale[,"Start"],unit.depths[t],tscale[,"End"],unit.depths[t+1], col=rgb(tscale[,"Col_R"], tscale[,"Col_G"], tscale[,"Col_B"], maxColorValue=255))
     ...
     par(mar=c(3,0,2,2))
      ...
      par(fig=c(ts.width,1,0,1),new=T)
        par(mar=c(3,0,2,2))
         ...
         par(lend=1); plot.phylo(tree,label.offset=offset,edge.width=width,no.margin=T,y.lim=x.lim,cex=cex.tip,direction="upwards",...)
          if (ranges == TRUE){
  ...
  # PLOT 1 - TIMESCALE
    ...
    par(mar=c(1,3,0,2))
      ...
      par(lend=1); plot.phylo(tree,plot=FALSE,label.offset=offset,no.margin=T,x.lim=x.lim,direction="rightwards",cex=cex.tip,...)
            
        timescale.names <- timescale.rescaled
         timescale.names <- timescale.names[timescale.names[,"End"] > min(c(par()$usr[1],par()$usr[2])) & timescale.names[,"Start"] < max(c(par()$usr[1],par()$usr[2])),]
          timescale.names[timescale.names[,"End"] > max(c(par()$usr[1],par()$usr[2])),"End"] <- max(c(par()$usr[1],par()$usr[2]))
          timescale.names[timescale.names[,"Start"] < min(c(par()$usr[1],par()$usr[2])),"Start"] <- min(c(par()$usr[1],par()$usr[2]))
    ...
    for(t in 1:length(units)){
 	    ...
 	    if(ts.col == TRUE & units[t] != "User"){rect(tscale[,"Start"],unit.depths[t],tscale[,"End"],unit.depths[t+1],col=rgb(tscale[,"Col_R"],tscale[,"Col_G"],tscale[,"Col_B"],maxColorValue=255))
          ...
          text(tscale.names[tscale.names[,"Type"] == units[t],"Midpoint"],(unit.depths[t] + unit.depths[t+1])/2,tscale.names[tscale.names[,"Type"] == units[t],"Name"],cex=cex.ts*tscale.data[match(units[t],rownames(tscale.data)),"size"],srt=tscale.data[match(units[t],rownames(tscale.data)),"srt"])
    }
  ...
  ## PLOT 2: PHYLOGENY
    ...
    par(mar=c(0,3,2,2))
      ...
      par(lend=1); plot.phylo(tree,plot=FALSE,label.offset=offset,no.margin=T,x.lim=x.lim,cex=cex.tip,...)
        lastPP <- get("last_plot.phylo", envir = .PlotPhyloEnv)
    ...
    par(mar=c(0,3,2,2))
      ...
      par(lend=1); plot.phylo(tree,label.offset=offset,edge.width=width,no.margin=T,x.lim=x.lim,cex=cex.tip,...)
        if (ranges == TRUE){
